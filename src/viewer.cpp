...
let playhead=0,playing=false,lastTime=0,playGeneration=0,playTimer=null,yaw=.42,pitch=-.9,zoom=1,drag=null,recorder=null,chunks=[];
...
function tick(generation){if(!playing||generation!==playGeneration)return;const now=performance.now(),dt=Math.min(100,Math.max(0,now-lastTime)),selectedSpeed=Number(speed.value),rate=Number.isFinite(selectedSpeed)&&selectedSpeed>0?selectedSpeed:1;lastTime=now;playhead+=dt*rate/650;if(playhead>=frames.length-1){playhead=frames.length-1;setPlaying(false);draw();finishRecording();return;}draw();playTimer=setTimeout(()=>tick(generation),16);}
function scrub(){setPlaying(false);const requested=Number(timeline.value);playhead=Number.isFinite(requested)?Math.max(0,Math.min(frames.length-1,requested)):0;draw();}
playButton.onclick=()=>{if(playing){setPlaying(false);return;}scrub();setPlaying(true);};resetButton.onclick=()=>{setPlaying(false);playhead=0;draw();};timeline.onpointerdown=()=>setPlaying(false);timeline.oninput=scrub;timeline.onchange=scrub;timeline.onpointerup=scrub;
